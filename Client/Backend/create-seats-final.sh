...
create_seats_for_room() {
    ...
    for row_char in $rows; do
        ...
        # Tạo INSERT statement cho hàng này
        local values=""
        for seat in $(seq 1 $seats_per_row); do
            if [ -n "$values" ]; then
                values="$values, "
            fi
            values="$values($room_id, '$row_char', $seat, '$seat_type', NOW(), NOW())"
        done
        
        # Chạy INSERT, bỏ qua lỗi duplicate
        mysql -u "$DB_USER" ${DB_PASS:+-p"$DB_PASS"} "$DB_NAME" -e "
            INSERT IGNORE INTO seats (room_id, \`row_number\`, seat_number, seat_type, created_at, updated_at)
            VALUES $values;
        " 2>/dev/null
    done
}
